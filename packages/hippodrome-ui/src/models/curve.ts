...
async function addLiquidityTx(
  ...
  if (token0Amt.lte(token1Amt)) {
    return curvePool["add_liquidity(uint256[2],uint256,bool)"](
      [token0Amt, token0Amt],
      0,
      true
    );
...
export const doAddLiquidity = async (
  ...
  const shifterApproveToken1 = await tokenContractFromAddress(
    token1.address
  ).populateTransaction.approve(shifter.address, constants.MaxUint256);
  const shifterApproveLPToken = await tokenContractFromAddress(
    lpTokenAddress
  ).populateTransaction.approve(shifter.address, constants.MaxUint256);

  const shiftTx = await shifter.populateTransaction.shift(
    [input, token0.address, token1.address, lpTokenAddress],
    safeAddress,
    address
  );
  ...
  const approves = await Promise.all([fetchApprove(input)]);

  const token0Approve = await tokenContractFromAddress(
    token0.address
  ).populateTransaction.approve(poolAddress, constants.MaxUint256);
  const token1Approve = await tokenContractFromAddress(
    token1.address
  ).populateTransaction.approve(poolAddress, constants.MaxUint256);

  console.log(
    "ren amount: ",
    renTx.out.amount.toString(),
    "half amount",
    halfSwap.toString()
  );
  ...
  await (
    await RenERC20LogicV1__factory.connect(token0.address, signer).approve(
      poolAddress,
      constants.MaxUint256
    )
  ).wait();
  await (
    await RenERC20LogicV1__factory.connect(token1.address, signer).approve(
      poolAddress,
      constants.MaxUint256
    )
  ).wait();
  ...
  console.log(
    "0 bal: ",
    (
      await RenERC20LogicV1__factory.connect(token0.address, signer).balanceOf(
        address
      )
    ).toString()
  );
  console.log(
    "01 bal: ",
    (
      await RenERC20LogicV1__factory.connect(token1.address, signer).balanceOf(
        address
      )
    ).toString()
  );
